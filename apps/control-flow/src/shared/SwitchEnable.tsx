...
export default function SwitchEnable(props: SwitchEnableProps) {
  ...
  const booleanToString = (newChecked: boolean): string =>
    newChecked ? "ENABLED" : "DISABLED";
  ...
  const handleSwitch = async (e: InputSwitchChangeEvent) => {
    ...
    const response = await props.callbackPatch!(
      props.configurationApi!,
      booleanToString(!checked),
    );
        ...
        <div className="flex align-items-center gap-3">
          ...
          {error && (
            <>
              <Tooltip
                target=".tooltip-msg"
                content="MIF API error"
                className="fadeinright animation-ease-out animation-duration-200"
              />
              <Message
                severity="error"
                className="tooltip-msg fadeinright animation-ease-out animation-duration-200 border-circle"
              />
            </>
          )}
        </div>
