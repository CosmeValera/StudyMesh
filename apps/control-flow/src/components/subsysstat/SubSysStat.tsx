...
export default function SubSysStat(props: { className?: string }) {
  ...
  const [MMsatStatMap, setMMsatStatMap] = useState<
    Record<string, string | boolean>
  >({});

  // Get control flow states from the provider
  const { sonarDataEnabled, acousticDataEnabled, telemetryDataEnabled } =
    useControlFlow();
  ...
  useEffect(() => {
    ...
    if (dataMm) {
      dataMm.forEach(
        (item: { key: string; value: string; readOnly: boolean }) => {
    ...
    if (activeSubsystems.length === 0) {
      buttons.push(
        <Button
          ...
          rounded
          className={styles["cf-button"]}
          onClick={() => {
            toast.current?.show({
              severity: "success",
              summary: "Comms Established",
              detail: <p>Communication relay successfully deployed!</p>,
              life: 3000,
            });
          }}
        />,
      ...
      buttons.push(
        <Button
          key="sensors"
          label="Activate Sensors"
          rounded
          className={styles["cf-button"]}
          onClick={() => {
            toast.current?.show({
              severity: "success",
              summary: "Sensors Activated",
              detail: <p>Acoustic signal transmitted, awaiting return data!</p>,
              life: 3000,
            });
          }}
        />,
      ...
      if (sonarDataEnabled) {
        buttons.push(
          <Button
            key="sonar"
            severity="success"
            label="Process Sonar Data"
            rounded
            className={`${styles["cf-button"]} ${styles.enhance}`}
            onClick={() => {
              toast.current?.show({
                severity: "success",
                summary: "Sonar Processing",
                detail: <p>Sonar data stream analysis in progress!</p>,
                life: 3000,
              });
            }}
          />,
      ...
      if (acousticDataEnabled) {
        buttons.push(
          <Button
            key="acoustic"
            severity="success"
            label="Analyze Acoustic Signals"
            rounded
            className={`${styles["cf-button"]} ${styles.enhance}`}
            onClick={() => {
              toast.current?.show({
                severity: "success",
                summary: "Acoustic Analysis",
                detail: (
                  <p>Acoustic signatures being processed and identified!</p>
                ),
                life: 3000,
              });
            }}
        ...
        buttons.push(
          <Button
            ...
            onClick={() => {
              toast.current?.show({
                severity: "success",
                summary: "Telemetry Monitoring",
                detail: <p>Real-time telemetry data stream established!</p>,
                life: 3000,
              });
            }}
          />,
    ...
    if (activeSubsystems.length === 0) {
      buttons.push(
        <Button
          key="emergency"
          severity="danger"
          label="Emergency Dive"
          rounded
          className={`${styles["cf-button"]} ${styles.danger}`}
          onClick={() => {
            toast.current?.show({
              severity: "warn",
              summary: "DANGER MODE",
              detail: <p>Emergency dive protocol initiated!</p>,
              life: 3000,
            });
          }}
        />,
        ...
        <Button
          ...
          onClick={() => {
            toast.current?.show({
              severity: "warn",
              summary: "DANGER MODE",
              detail: <p>Acoustic countermeasures deployed!</p>,
              life: 3000,
            });
          }}
        />,
          ...
          <Button
            ...
            onClick={() => {
              toast.current?.show({
                severity: "warn",
                summary: "DANGER MODE",
                detail: <p>Sonar in passive detection mode only!</p>,
                life: 3000,
              });
            }}
          />,
          ...
          <Button
            ...
            onClick={() => {
              toast.current?.show({
                severity: "warn",
                summary: "DANGER MODE",
                detail: <p>Initiating acoustic jamming sequence!</p>,
                life: 3000,
              });
            }}
          />,
      ...
      if (telemetryDataEnabled) {
        buttons.push(
          <Button
            key="telemetry-danger"
            severity="danger"
            label="Encrypt Telemetry"
            rounded
            className={`${styles["cf-button"]} ${styles.danger}`}
            onClick={() => {
              toast.current?.show({
                severity: "warn",
                summary: "DANGER MODE",
                detail: <p>All telemetry streams now fully encrypted!</p>,
                life: 3000,
              });
            }}
          />,
  ...
  return (
    <div className={`card ${props.className}`}>
      <Fieldset
        legend="SubSysStat Configuration"
        toggleable
        className="m-2"
        pt={{
      ...
      >
        ...
        />
        <div className="flex gap-6 mt-3 flex-wrap">{renderButtons()}</div>
      </Fieldset>
    </div>
  );
}
