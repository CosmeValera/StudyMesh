...
export default function Ops(props: { className?: string }) {
  ...
  const [isStmProcessingChecked, setIsStmProcessingChecked] = useState<boolean>(
    !!stmMap["MAIN_PROCESSING"],
  );
  ...
  const handleMainProcessingSwitchChange = async (
    key: string,
    value: boolean,
  ) => {
