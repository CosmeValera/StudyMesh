...
describe('EditorCanvas first-widget guide', () => {
  ...
  it('shows starter actions for an empty editable widget', () => {
    ...
    expect(
      screen.getByText('Build a reusable study widget'),
    ).toBeInTheDocument()
    expect(
      screen.getByRole('button', { name: /study summary widget/i }),
    ).toBeInTheDocument()
    expect(
      screen.getByRole('button', { name: /chart block/i }),
    ).toBeInTheDocument()
    expect(
      screen.getByRole('button', { name: /study note/i }),
    ).toBeInTheDocument()
    expect(
      screen.getByRole('button', { name: /answer field/i }),
    ).toBeInTheDocument()
    expect(
  ...
  it('uses existing starter handlers', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /study note/i }))
    fireEvent.click(screen.getByRole('button', { name: /answer field/i }))
